# Issue: Windows Crash on Startup (State Initialization Race Condition)

## Description

The application crashes immediately upon launch on Windows systems with the error `state() called before manage()`, while functioning correctly on macOS.

This is caused by a race condition in the Tauri lifecycle. On Windows, background tasks (like the Tray Icon event loop or Plugins) are firing events and attempting to access `StoreManager` **before** the `.setup()` hook has finished running.

## Error Log

```text
thread 'tokio-runtime-worker' panicked at ...
state() called before manage() for copilot_tracker::store::StoreManager

```

## Solution

We must move the `StoreManager` initialization out of the `.setup()` hook and into the `main()` function, passing it to `.manage()` immediately. This ensures the state exists before any background threads or plugins start.

Since `StoreManager` requires a path (which usually requires a running AppHandle), we will calculate the App Data directory manually in `main()` using the application context.

## Code Changes

### 1. Update `src-tauri/src/store.rs`

We need to remove the `init_store_manager` function (which causes the late management) and move the directory creation logic into `StoreManager::new`.

**Replace the entire content of `src-tauri/src/store.rs` with this:**
...
We introduce a helper to calculate the path and initialize `StoreManager` inside `main()` before the builder runs.

**Replace the entire content of `src-tauri/src/main.rs` with this:**
...
// Removed init_store_manager from import
use copilot_tracker::{
...
fn main() {
    ...
    tauri::Builder::default()
        // Manage state (CRITICAL FIX: Managed here, not in setup)
...
Because we removed `init_store_manager` from `store.rs`, your `lib.rs` will now fail to compile (since it tries to export a function that no longer exists).

You need to update `src-tauri/src/lib.rs` to remove `init_store_manager` from the export list.

### 3. Update `src-tauri/src/lib.rs`

**Replace the content of `src-tauri/src/lib.rs` with this:**
...
### Summary of Changes (Copy & Paste Order)

1. **Paste** the new code into `src-tauri/src/store.rs` (moves directory creation logic to `new()`).
2. **Paste** the new code into `src-tauri/src/main.rs` (moves initialization before the builder).
3. **Paste** the small fix above into `src-tauri/src/lib.rs` (removes the dead export).

Once these three files are saved, run `npm run tauri build` (or `cargo tauri build`) and the Windows crash will be gone.