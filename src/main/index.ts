import { app, shell, BrowserWindow, ipcMain, Tray, Menu, nativeImage, BrowserView } from 'electron'
...
// Constants
const GITHUB_BILLING_URL = 'https://github.com/settings/billing/premium_requests_usage'
...
let authView: BrowserView | null = null
...
function updateTrayMenu(usage?: CopilotUsage | null): void {
  ...
  if (usage) {
    tray.setToolTip(`Copilot: ${Math.round(usage.discountQuantity)}/${usage.userPremiumRequestEntitlement}`)
  }
}
...
async function fetchUsageData(): Promise<void> {
  ...
  try {
    ...
    if (!usageParsed.success) {
      mainWindow.webContents.send('usage:data', { success: false, error: usageParsed.error })
      return
    }
    ...
    const usage: CopilotUsage = {
      ...
      userPremiumRequestEntitlement: usageData.user_premium_request_entitlement ?? 0,
      filteredUserPremiumRequestEntitlement: usageData.filtered_user_premium_request_entitlement ?? 0
    }

    // Parse history data
    const historyData = historyParsed.success ? historyParsed.data : { rows: [] }
    const history = {
      fetchedAt: new Date().toISOString(),
      days: (historyData.rows || []).map((row: { date: string; included_requests?: number; billed_requests?: number; gross_amount?: number; billed_amount?: number }) => ({
        date: row.date,
    ...
    // Cache data
    store.set('cache', { usage, history, lastFetched: new Date().toISOString() })
...
function setupIpcHandlers(): void {
  ...
  ipcMain.on('auth:logout', () => {
    if (authView) {
      authView.webContents.session.clearStorageData()
      customerId = null
      mainWindow?.webContents.send('auth:state-changed', 'unauthenticated')
    }
  })
