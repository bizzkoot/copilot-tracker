...
export function Dashboard() {
  const { usage, history, prediction, isLoading, error, lastUpdated, refresh } = useUsage()

  // Calculate daily average for chart reference line
  const dailyAverage = history && history.days.length > 0
    ? history.days.reduce((sum, day) => sum + day.includedRequests + day.billedRequests, 0) / history.days.length
    : undefined
      ...
      {error && (
        <div className="p-4 rounded-lg bg-destructive/10 border border-destructive/20 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="font-medium text-destructive">Failed to load usage data</p>
