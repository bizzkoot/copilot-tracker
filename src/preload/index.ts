...
const electronAPI = {
  ...
  onAuthStateChanged: (callback: (state: string) => void): (() => void) => {
    const listener = (_event: Electron.IpcRendererEvent, state: string): void => callback(state)
  ...
  onUsageLoading: (callback: (loading: boolean) => void): (() => void) => {
    const listener = (_event: Electron.IpcRendererEvent, loading: boolean): void => callback(loading)
  ...
  setSettings: (settings: unknown): Promise<void> => ipcRenderer.invoke('settings:set', settings),
  resetSettings: (): Promise<void> => ipcRenderer.invoke('settings:reset'),
  onSettingsChanged: (callback: (settings: unknown) => void): (() => void) => {
    const listener = (_event: Electron.IpcRendererEvent, settings: unknown): void => callback(settings)
